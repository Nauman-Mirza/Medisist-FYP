import "./update_doctor_form.css";

import AuthUser from "../../http/https";
import { useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
function DoctorUpdateForm(props) {
  ...
  const fetchUser = () => {
    http.get("/admins/adminDoctor/" + id + "/edit").then((res) => {
      setInputs({
  ...
  const submitForm = () => {
    ...
    http.put("/admins/adminDoctor/update/" + id, data).then((res) => {
      navigate("/admin_doctor_data");
    });
  };
  return (
    <div className="pat_update">
      <div className="a_pat_update_title">
        <h1>Doctor's Edit Form</h1>
      </div>
